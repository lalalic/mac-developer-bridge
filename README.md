...
Two runtime-only options support dedicated Family Tutor tabs. `bootstrap_prompt` requires an exact `project_id` and `tab_id`; MDB submits it only when that exact tab is mounted on the Project home/new-thread state, and otherwise submits `prompt`. This makes project bootstrap/memory first-turn-only instead of replaying it after a bridge restart. `preserve_tab`, false by default, also requires that exact Project/tab pair and refuses navigation if the mounted route is not the Project home or a conversation within that Project. On completion MDB verifies the same mounted assistant message id and exact text against the runtime result without `chrome.tabs.reload`, failing closed on any mismatch. Calls that omit `preserve_tab` keep the existing behavior: lease/release a managed tab, reload the returned conversation, and read its persisted assistant message before returning.
