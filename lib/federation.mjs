...
class Provider {
  ...
  registerTools({ firstStart }) {
    ...
    for (const [name] of next) {
      if (!this.toolsByPrefixed.has(name)) this.host.claimToolName(name, this.key);
    }
    ...
    if (vanished.length > 0 || appeared.length > 0) this.host.notifyToolsChanged?.();
  }
  ...
  async refreshTools() {
    ...
    this.toolsRefreshPromise = (async () => {
      this.tools = await this.fetchTools({ startup: false });
      this.registerTools({ firstStart: false });
    })().finally(() => {
...
class HttpProvider extends Provider {
  ...
  async readResponseMessages(response, contentType, expectedId) {
    ...
    if (final === undefined && expectedId !== undefined) throw new Error("HTTP MCP response did not contain the requested JSON-RPC response");
    return final;
  }
